...
import (
	...
	"github.com/pavlo67/punctum/interfaces/validator"
...
func (h *ImporterHTML) Init(feedURL, importParams string, testMode bool) error {
	...
	if len(arr) > 1 {
		h.httpS = arr[1]
		//log.Println("IS http protocol:", arr[1])
	} else {
	...
	if len(arr) > 1 {
		h.title = arr[1]
		//log.Println("IS title:", arr[1])
	}
	arr = reMeta.FindStringSubmatch(h.content)
	if len(arr) > 1 {
		h.meta = arr[1]
		log.Println("IS meta:", arr[1])
	}
	...
	if len(h.params.AcceptableTags) > 0 && h.params.AcceptableTags[0] != "" {
		...
		for t := range allTags {
			...
			if needDrop {
				//log.Println("IS tag for clear:", t)
...
func (h *ImporterHTML) Next() (importer.Entity, error) {
	...
	if h.itemIndex < len(h.items) {
		...
		if title == "" {
			...
			title = basis.ReSpaces.ReplaceAllString(title, " ")
		}
		if len(title) > 255 {
			title = title[:250]
		}
...
func (e Entity) Object() (*interfaces.Object, error) {
	o := interfaces.Object{
		Name:     e.mapObject["title"],
		Content:  e.mapObject["content"],
		GlobalIS: validator.RandomString(60), // todo!!! temporary
